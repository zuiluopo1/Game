                ...
                for (int j = 0; j < table.Columns.Count; j++)
                {
                    switch (rowType[j].ToString())
                    {
                        case "int":
                            ...
                            {
                                Debug.LogError($"无法解析 '{row[j]}' 为整数，行 {i} 列 {j}，默认设为 0");
                                fs.Write(BitConverter.GetBytes(0), 0, 4); // 设为默认值 0
                            }
                            break;
                        case "float":
                            ...
                            {
                                Debug.LogError($"无法解析 '{row[j]}' 为浮点数，行 {i} 列 {j}，默认设为 0");
                                fs.Write(BitConverter.GetBytes(0f), 0, 4);
                            }
                            break;
                        case "bool":
                            if (bool.TryParse(row[j].ToString(), out bool boolValue))
                                fs.Write(BitConverter.GetBytes(boolValue), 0, 1);
                            else
                            {
                                Debug.LogError($"无法解析 '{row[j]}' 为布尔值，行 {i} 列 {j}，默认设为 false");
